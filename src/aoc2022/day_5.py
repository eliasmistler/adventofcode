...
def apply_instruction(instruction: Instruction, stacks: Stacks, bulk_move: bool) -> Stacks:
    ...
    return toolz.merge(
        stacks,
        {
            instruction.from_stack: stacks[instruction.from_stack][:-instruction.n_crates],
            instruction.to_stack: [
                *stacks[instruction.to_stack],
                *moved_crates
            ]
        }
    )
...
def apply_instructions(instructions: list[Instruction], stacks: Stacks, bulk_move: bool) -> Stacks:
